...
def merge_exp_file_p30(all_files): #Change here for different percentiles
    frame = DataFrame()
...
def act_pred_merge_p30(pred_data,act_data): #Change here for different percentiles
    ...
    pred_data = pred_data.drop(['item_id','p40','p50'], axis = 1) #Change here for different percentiles
    pred_data['p30'] = round(pred_data['p30'], 7) #Change here for different percentiles
...
def calculating_mape_p30(bucket_name, file_key, exp_file_loc, actuals_df, forecast_horizon_len, model_name='ETS'): #Change here for different percentiles
    ...
    merged_frame = merge_exp_file_p30(list_f) #Change here for different percentiles
    df_act_prd = act_pred_merge_p30(merged_frame, actuals_df) #Change here for different percentiles
    # MAPE calculation
    list_mape = []
    list_product = []
    df_act_prd['p30'] = df_act_prd['p30'].round().astype(int) #Change here for different percentiles
    df_act_prd['p30'] = np.where(df_act_prd['p30'] < 0, 0, df_act_prd['p30']) #Change here for different percentiles
    ...
    for p in df_act_prd["(L0) Product"].unique():
        df_pred = df_act_prd[df_act_prd["(L0) Product"] == p]["p30"].to_list() #Change here for different percentiles
...
#                 Here, if the mape exceeds 100%, we are replacing it with 100%
    ...
    monthly_ape2['model'] = model_name +"_P30"   #Changed name here
    monthly_ape2
